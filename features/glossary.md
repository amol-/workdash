...
- CI result: The combined state of the checks GitHub last ran on an authored pull request's most recent commit, shown as the first one-character symbol in the Type column. A pull request with no checks configured has no CI result.
- Review decision: GitHub's rollup verdict on an authored pull request's reviews (for example approved, or changes requested), shown as the second one-character symbol in the Type column, independent of the CI result symbol. Approved shows a green checkmark, changes requested shows a red cross, a reviewer requested but not yet decided shows a question mark, and no reviewer requested shows a blank.
