...
from .repo_resolver import build_tracked_search_scope, validate_repository_selectors
...
class WorkdashBackend:
    ...
    def load_items(
        ...
        selectors = validate_repository_selectors(list(self.config.repositories))
        report_progress(f"Tracking {len(selectors)} repository selector(s):")
        for selector in selectors:
            report_progress(f"  - {selector}")
        tracked_search_scope = build_tracked_search_scope(selectors)
        github_username = self.config.github_username

        report_progress("Fetching open authored pull requests...")
        authored_pull_requests = self.github_client.list_open_authored_prs(
            github_username, progress_callback=report_progress
        )
        ...
        recent_tracked_items = self.github_client.list_recent_tracked_items(
            tracked_search_scope, progress_callback=report_progress
        )
        report_progress(f"Fetched {len(recent_tracked_items)} recent tracked item(s).")

        report_progress("Fetching open todo issues...")
        todo_issues = self.github_client.list_open_todo_issues(
            self.config.todo_repository, progress_callback=report_progress
        )
